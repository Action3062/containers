...
elif [[ "${channel}" == "catalog-storage-offload" ]]; then
    version=$(curl -sX GET "https://api.github.com/repos/Action3062/nzbdav/commits/feature/nzb-external-storage" --header "Authorization: Bearer ${TOKEN}" | jq --raw-output '.sha')
else
